...
describe LineAnalyzer do
  ...
  context "#calculate_word_frequency" do
    subject(:lineAnalyzer) { LineAnalyzer.new("Will it work maybe it will work do you think it will it will", 2 )}
    
    it "highest_wf_count value is 4" do
      expect(lineAnalyzer.highest_wf_count).to eq(4)
    end
    it "highest_wf_words will include \"will\" and \"it\"" do
      expect(lineAnalyzer.highest_wf_words).to include "will", "it"
    end
  end
end